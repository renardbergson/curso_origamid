...
const frutas: string[] = ["Maçã", "Banana", "Morango"];
frutas.push("Cacau");
...
const roles: readonly [string, string, string] = ["admin", "user", "guest"];
...
// B) Com `as const`
function pegarTexto2(seletor: string) {
  const elemento = document.querySelector<HTMLElement>(seletor);
  if (elemento) {
