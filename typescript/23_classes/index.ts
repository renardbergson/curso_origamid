...
class Produto {
  tipo = "Produto";
  nome: string;
  preco: number | undefined; // "preco" é opcional.
...
console.log(
  `Tipo: ${livro.tipo}; Nome: ${livro.nome}; Preço: R$ ${livro.preco}`
);
...
class Produto2 {
  readonly tipo = "Produto"; // Apenas leitura.
  protected preco: number; // Acessível apenas na classe e subclasses.
  private nome: string; // Acessível apenas dentro da classe.
...
const formas = [2, 32, 12, 3, 4, 20, 37, 9].map((n) =>
  n < 15 ? new Quadrado(n) : new Circulo(n)
);
