...
// Também é possível definir a função antes de implementá-la, mas é raro
type Callback = (event: MouseEvent) => void; // Interface para uma função que lida com cliques
...
// Overloads:
// Overloads são usados para funções que precisam de múltiplas assinaturas diferentes.
// Por exemplo, uma função que aceita diferentes tipos de argumentos ou retorna
// tipos distintos, dependendo de como é chamada. Use overloads para capturar essas variações.
