...
async function fetchVendas() {
  ...
  if (vendas) mostrarNaTela(vendas.join(""));
}
...
type Venda = [string, number, string, DetalhesVenda];
...
function handleData(data: unknown): string[] | undefined {
  if (isVenda(data)) {
    let totalVendas = 0;
...
function mostrarNaTela(vendas: string): void {
